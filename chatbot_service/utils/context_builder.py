...
class ContextBuilder:
    ...
    def build_prompt(
        ...
        prompt += (
            "Nếu thông tin không đầy đủ, hãy thừa nhận giới hạn và tránh suy đoán. "
        )
        prompt += "Phản hồi nên được định dạng rõ ràng, dễ đọc với đoạn văn ngắn gọn và mạch lạc."

        logger.debug(
            f"Built prompt with {len(retrieved_docs)} retrieved docs and {len(conversation_history) if conversation_history else 0} conversation history messages"
        )
