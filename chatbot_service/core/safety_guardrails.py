...
class SafetyGuardrails:
    def __init__(self, max_risk_level: int = 3):
        ...
        self.high_risk_keywords = [
            "tự tử", "giết", "đau khổ", "thuốc bất hợp pháp", "ma túy", 
            "vũ khí", "bom", "tấn công", "nguy hiểm", "cách chế tạo"
        ]
        
        # Từ khóa y tế cần disclaimer
        self.medical_keywords = [
            "điều trị", "thuốc", "liều lượng", "tác dụng phụ", "bệnh", "triệu chứng",
            "chẩn đoán", "nguyên nhân", "phòng ngừa"
        ]
    ...
    async def check_input_safety(self, message: ChatMessage) -> Tuple[bool, int, str]:
        ...
        for keyword in self.high_risk_keywords:
            if keyword in content:
                ...
                if risk_level > self.max_risk_level:
                    return False, risk_level, f"Nội dung chứa từ khóa có rủi ro cao: {keyword}"
                else:
        ...
        if re.search(r'(làm thế nào|cách) (để|tạo ra|chế tạo|sản xuất) (vũ khí|bom|chất độc|thuốc)', content):
    ...
    async def check_output_safety(self, content: str) -> Tuple[bool, int, str]:
        ...
        if re.search(r'(cách|hướng dẫn|bước) (tạo ra|chế tạo|sản xuất) (vũ khí|bom|chất độc|thuốc)', content_lower):
            return False, 5, "Phản hồi chứa hướng dẫn tạo vật nguy hiểm"
        
        # Kiểm tra nội dung chứa lời khuyên y tế không đảm bảo
        if "tôi đảm bảo" in content_lower and any(kw in content_lower for kw in self.medical_keywords):
            return False, 4, "Phản hồi đưa ra đảm bảo y tế không phù hợp"
        
        # Kiểm tra nội dung khuyên dừng điều trị
        if re.search(r'(nên|hãy) (dừng|ngừng|bỏ) (điều trị|thuốc|liệu pháp)', content_lower):
    ...
    async def add_medical_disclaimer(self, content: str, risk_level: int = 0) -> str:
        ...
        is_medical_content = any(keyword in content.lower() for keyword in self.medical_keywords)
    ...
    async def classify_risk_level(self, content: str) -> int:
        ...
        # Mức 5: Nguy hiểm cao
        if any(re.search(rf'\b{re.escape(kw)}\b', content_lower) for kw in ["tự tử", "giết người", "bom"]):
            return 5
        ...
        if any(re.search(rf'\b{re.escape(kw)}\b', content_lower) for kw in ["liều lượng", "tác dụng phụ", "ngừng thuốc"]):
            return 3
