...
class SourceCitationService:
    ...
    async def extract_sources(
        self, content: str, retrieved_docs: List[Dict[str, Any]]
    ) -> List[Source]:
        """
            ...
            if not self._is_duplicate_source(sources, metadata):
                source = Source(
                    ...
                    description=metadata.get(
                        "description", self._create_snippet(doc.get("content", ""), 100)
                    ),
    ...
    def _is_duplicate_source(
        self, sources: List[Source], metadata: Dict[str, Any]
    ) -> bool:
        """
        Kiểm tra xem nguồn đã tồn tại trong danh sách chưa
        """
        for source in sources:
            if source.title == metadata.get("title") or source.url == metadata.get(
                "url"
            ):
                return True
        return False
