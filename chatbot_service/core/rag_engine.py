...
class RagEngine:
    ...
    async def process_message(self, message: ChatMessage, conversation_history: List[ChatMessage] = None) -> MessageResponse:
        """
        ...
        is_safe, risk_level, reason = await self.safety_guardrails.check_input_safety(message)
        if not is_safe:
        ...
        is_emergency, emergency_type = await self.emergency_detector.detect_emergency(message.content)
        if is_emergency:
        ...
        is_safe_output, risk_level_output, reason_output = await self.safety_guardrails.check_output_safety(raw_response)
        ...
        response_with_disclaimer = await self.safety_guardrails.add_medical_disclaimer(
            raw_response,
            risk_level=max(risk_level, risk_level_output)
        )
        
        # Trích xuất nguồn tham khảo
        sources = await self.source_citation.extract_sources(response_with_disclaimer, relevant_docs)
        ...
        return MessageResponse(
            ...
            sources=sources,
            intent={"primary_intent": "medical_query", "confidence": 0.95},  # placeholder
            suggestions=suggestions,
            timestamp=message.created_at or datetime.utcnow()
        )
    
    def _create_safety_response(self, message: ChatMessage, reason: str) -> MessageResponse:
        """
        Tạo phản hồi an toàn khi phát hiện nội dung không phù hợp
        """
        response = "Xin lỗi, tôi không thể cung cấp thông tin cho truy vấn này. " \
        ...
        return MessageResponse(
            ...
            suggestions=["Tôi có thể hỏi về triệu chứng của cảm cúm không?", 
                        "Làm thế nào để duy trì lối sống lành mạnh?"],
            timestamp=message.created_at or datetime.utcnow()
        )
    
    async def _handle_emergency(self, message: ChatMessage, emergency_type: str) -> MessageResponse:
        """
        Xử lý tình huống khẩn cấp
        """
        # Tạo phản hồi khẩn cấp với hướng dẫn cụ thể
        emergency_response = f"ĐÂY CÓ VẺ LÀ TÌNH HUỐNG KHẨN CẤP. Nếu bạn hoặc người khác đang gặp nguy hiểm, " \
                            f"hãy gọi ngay số cấp cứu 115 hoặc đến cơ sở y tế gần nhất. "
        
        if emergency_type == "cardiac":
            emergency_response += "Dấu hiệu đau tim cần được xử lý ngay lập tức bởi chuyên gia y tế."
        ...
        return MessageResponse(
            ...
            suggestions=["Làm thế nào để nhận biết dấu hiệu đau tim?", 
